...
#################################################
### Fonctions de calcul de lois normales de base
#################################################

def pdf(x): 
    '''
    Retourne la densité de probabilité de la loi normale centrée réduite en x
    '''
    return norm.pdf(x)
...
def cdf(x): 
    '''
    Retourne la fonction de répartion de la loi normale centrée réduite en x
    '''
    return norm.cdf(x) ##Phi 

def ppf(x): 
    '''
    Retourne la fonction de répartion inverse de la loi normale centrée réduite en x
    '''
    return norm.ppf(x) ##Phi^-1

def randn(mu=0, sigma=1, size=1): 
    '''
    Retourne un vecteur de taille size d'une loi normale N(mu, sigma)
    '''
    return mu + sqrt(sigma)*np.random.randn(size) ## N(0,1)

def lognormale(mu=0, sigma=1, size=1) : 
    '''
    Retourne un vecteur de taille size d'une loi lognormale LN(mu, sigma)
    '''
...
def time(func):
    ...
    Paramètres : 
    calcul_time : bool, print le temps de calcul si calcul_time == True
    '''
...
class Credit_model:
    '''
    Crée une classe de modèle de crédit
    ...
    def simulate_notes(self,notes,X=1) : 
        ...
        rho = self.rho
        matrixC = self.matrixC
        Zs = sqrt(rho)*X + sqrt(1-rho)*randn(size = len(notes))  ## Simulation des Z
        phi_Zs = 1 - cdf(Zs) ## Simulation des phi_Z
        phi_Zs = phi_Zs.reshape((len(notes),1))
        return np.argmax(phi_Zs<=matrixC[notes], axis = 1) ###Retourne les notes à 1 an asociées
    ...
    def check_question2(self, n0=0, n1an=1, X=0, N=10000): 
        ...
        theorique_k = cdf(1/sqrt(1-rho)*(ppf(matrixCi[n1an]) + sqrt(rho)*X))
        theorique_k1 = 0 if n1an == 0 else cdf(1/sqrt(1-rho)*(ppf(matrixCi[n1an-1]) + sqrt(rho)*X))
    ...
    def one_perte(self,nb_companies=1000,notes=None,E=None,X=None,mu=0,sigma=1): 
        ...
        leurs notes et leurs positions à t=0
        ...
        E :     vecteur de taille nb_companies de positions (réels). S'il n'est pas renseigné, 
        ...
        if E is not None : 
            assert len(E)==nb_companies, "Le nombre de positions n'est pas égal au nombre d'entreprises"
        else : 
            E = lognormale(mu,sigma,nb_companies) # Si les positions ne sont pas renseignées : random
                
        if X is None : 
            X = randn() # Variable d'état économique
...
if __name__ == '__main__' : 
    ...
    model = Credit_model(rho=rho, matrixM=matrixM)
    ...
    print('###################')
    print('Pour le calcul des pertes, on suppose que X, les notes à t = 0 et les positions sont aléatoires\npour chaque simulation\n')    
    ...
    print('###################')
    print('Pour le calcul des pertes, on suppose que X, les notes à t = 0 et les positions sont fixées une bonne fois pour toute')    
    nb_simulations = 10_000
    nb_companies=1000
    alpha = 0.65
    X = randn()
    notes = np.random.randint(3, size = nb_companies)
    E = lognormale(size = nb_companies)
